...
const tag_save_1 = require("../action/tag-save");
class IntentList {
    constructor(intents) {
        this.validators = [];
        ...
        for (const intent of intents) {
            ...
            for (const action of intent.toActions()) {
                ...
                if (action instanceof action_1.EntityAction) {
                    ...
                    if (entityType === action_1.EntityType.ContentType) {
                        const ct = await api.getContentType(entityId);
                        await action.applyTo(ct);
                    }
                    if (entityType === action_1.EntityType.EditorInterface) {
                        try {
                    ...
                    if (entityType === action_1.EntityType.Tag) {
            ...
            if (intent.shouldSave()) {
                ...
                if (intent.isTagIntent()) {
                    save = new tag_save_1.TagSaveAction(intent.getTagId());
                }
                else {
                    save = new content_type_save_1.ContentTypeSaveAction(intent.getContentTypeId());
                }
