...
exports.ParseResult = exports.createMigrationParser = exports.default = void 0;
...
const tag_update_1 = __importDefault(require("./intent-validator/tag-update"));
...
const createMigrationParser = function (makeRequest, config) {
    return async function migration(migrationCreator) {
        const fetcher = new fetcher_1.default(makeRequest, config.requestBatchSize);
        const parseResult = new ParseResult();
        const intents = await (0, migration_steps_1.migration)(migrationCreator, makeRequest, config);
        ...
        intentList.addValidator(new tag_update_1.default());
        ...
        for (const apiTag of apiTags) {
            const tag = new tag_1.default(apiTag);
            existingTags.set(tag.id, tag);
        }
        const tags = apiTags.map((apiTag) => {
            return new tag_1.default(apiTag);
        });
        const payloadValidationErrors = (0, validation_1.default)(intentList, ctsWithEntryInfo, tags);
