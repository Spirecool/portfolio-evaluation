...
function mergeSections(sections) {
    const sameSections = (0, lodash_1.groupBy)(sections, 'heading');
    const mergedSections = [];
    for (const [heading, sections] of (0, lodash_1.entries)(sameSections)) {
        const details = (0, lodash_1.flatten)(sections.map((section) => section.details || []));
...
class ComposedIntent {
    ...
    toActions() {
        return (0, lodash_1.flatten)(this.intents.map((intent) => intent.toActions()));
    }
    toPlanMessage() {
        ...
        const onlyFieldUpdates = (0, lodash_1.difference)(fieldUpdates, createdFieldUpdates);
        const onlyFieldUpdatesByField = (0, lodash_1.groupBy)(onlyFieldUpdates, (intent) => intent.getFieldId());
        const createdFieldUpdatesByField = (0, lodash_1.groupBy)(createdFieldUpdates, (intent) => intent.getFieldId());
        const topLevelDetails = (0, lodash_1.flatten)(contentTypeOrTagUpdates.map((updateIntent) => updateIntent.toPlanMessage().details));
        const sidebarUpdates = (0, lodash_1.flatten)(this.intents
        ...
        for (const createIntent of fieldCreates) {
            ...
            const allFieldUpdateSections = (0, lodash_1.flatten)(updateIntents.map((fieldIntent) => fieldIntent.toPlanMessage().sections));
        ...
        for (const updateIntents of (0, lodash_1.values)(onlyFieldUpdatesByField)) {
            const allSections = (0, lodash_1.flatten)(updateIntents.map((intent) => intent.toPlanMessage().sections));
