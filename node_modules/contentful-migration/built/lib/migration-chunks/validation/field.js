...
const fieldErrors = errors_1.default.field;
const deriveErrors = errors_1.default.entry.derivation;
const transformErrors = errors_1.default.entry.transformation;
...
const checks = {
    'field/create': (errors, intent, validationContext) => {
    ...
    'field/update': (errors, intent, validationContext) => {
        ...
        if (changedId) {
            errors.push(invalidActionError(fieldErrors.changeId.REFERENCE_TO_OLD_ID(id, changedId), intent));
        }
    ...
    'field/move': (errors, intent, validationContext) => {
        ...
        if (isRelativeMovement) {
            ...
            if (!pivotExists && !pivotRemoved) {
                errors.push(invalidActionError(fieldErrors.move.PIVOT_FIELD_DOES_NOT_EXIST(intent.getPivotId(), direction), intent));
            }
            if (!pivotExists && pivotRemoved) {
                errors.push(invalidActionError(fieldErrors.move.PIVOT_FIELD_DELETED(intent.getPivotId(), direction), intent));
            }
        }
        if (!exists && removed) {
    ...
    'field/rename': (errors, intent, validationContext) => {
        ...
        if (oldId === newId) {
            ...
            errors.push(invalidActionError(fieldErrors.changeId.ID_ALREADY_EXISTS(oldId, newId, intent.getContentTypeId()), intent));
        }
    ...
    for (const intent of intents) {
        ...
        if (intent.isAboutField()) {
            ...
            if (!contentTypeExists) {
                ...
                errors.push(invalidActionError(fieldErrors[type].CONTENT_TYPE_DOES_NOT_EXIST(fieldId, contentTypeId), intent));
                continue;
            }
